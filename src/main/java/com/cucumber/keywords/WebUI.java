...
import com.cucumber.enums.FailureHandling;
...
import com.cucumber.report.ExtentTestManager;
import com.cucumber.utils.BrowserInfoUtils;
import com.cucumber.utils.DateUtils;
        ...
        try {
            ...
            for (int i = 0; i < Objects.requireNonNull(listOfFiles).length; i++) {
                if (listOfFiles[i].isFile()) {
                    boolean result = new File(listOfFiles[i].toString()).delete();
                }
            }
        } catch (Exception e) {
            e.getMessage();
